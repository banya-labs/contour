...
export default function AccountLockedPage({ searchParams }: { searchParams: { status?: string } }) {
  const status = searchParams.status === "SUSPENDED" ? "suspended" : searchParams.status === "DELETION_PENDING" ? "scheduled for review" : "temporarily locked";
