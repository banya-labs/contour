...
export function DriveFileBrowser() {
  ...
  const getCategoryBadge = (file: DriveFile) => {
    if (file.listing) {
      return {
        label: `Property: ${file.listing.title}`,
        color: "bg-blue-100 text-blue-700",
      };
    }
    if (file.deal) {
      return {
        label: `Deal: ${file.deal.title}`,
        color: "bg-green-100 text-green-700",
      };
    }
    if (file.client) {
      return {
        label: `Client: ${file.client.fullName}`,
        color: "bg-purple-100 text-purple-700",
      };
    }
    if (file.lease) {
      return {
        label: `Lease: ${file.lease.leaseName}`,
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 overflow-hidden rounded-[20px] border border-[color:var(--border)] bg-[color:var(--surface)] p-4">
      ...
      {folders.length > 0 && (
        <div>
          <p className="mb-2 text-[11px] uppercase tracking-wider text-[color:var(--muted)]">
            Folders
          </p>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {folders.map((folder) => (
              ...
              >
                ...
                <p className="truncate text-[11px] font-medium text-[color:var(--foreground)]">
                  {folder.name}
                </p>
              ...
              return (
                ...
                >
                  <div className="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2">
                        {category && (
                          <span
                            className={`rounded-full px-2 py-1 text-[10px] font-medium ${category.color}`}
                          >
                            {category.label}
                          </span>
                        )}
                  ...
                  <div className="flex items-center gap-1">
                    <button className="rounded-full p-2 transition-colors hover:bg-[color:var(--surface)]">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-[12px] text-[color:var(--muted)]">
              Upload files to organize your workspace documents
            </p>
          </div>
        </div>
      )}
