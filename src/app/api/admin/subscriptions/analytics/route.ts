...
export async function GET(request: NextRequest) {
  ...
  const tiers = Object.values(CONTOUR_PLANS).map((plan) => {
    ...
    return { id: plan.id, name: plan.name, description: plan.description, monthlyZmw: plan.monthlyZmw, annualZmw: plan.annualZmw, monthlyUsd: plan.monthlyUsd, annualUsd: plan.annualUsd, active: true, subscriberCount: subscribers.length, activeCount: subscribers.filter((item) => item.subscriptionStatus === "active").length, trialCount: subscribers.filter((item) => item.subscriptionStatus === "trialing").length, pastDueCount: subscribers.filter((item) => item.subscriptionStatus === "past_due").length, grossCollected: collected };
  });
