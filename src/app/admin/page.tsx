...
export default function AdminPage() {
  const [period, setPeriod] = useState<AdminPeriod>("this_month");
  const [overview, setOverview] = useState<{ totalAgencies: number; trialing: number; churnRate: number } | null>(null);
  const [subscription, setSubscription] = useState<{ mrrZmw: number; arrZmw: number; activeSubscribers: number } | null>(null);
  useEffect(() => { void Promise.all([fetch(`/api/admin/agencies/analytics?period=${period}`).then((response) => response.json()), fetch(`/api/admin/subscriptions/analytics?period=${period}`).then((response) => response.json())]).then(([agencies, subscriptions]) => { setOverview(agencies.stats || null); setSubscription(subscriptions.totals || null); }); }, [period]);
  return (
    <main className="min-h-screen bg-editorial-bg px-4 py-5 font-geist text-editorial-black sm:px-8 lg:px-12">
      <div className="mx-auto w-full max-w-7xl space-y-8 pb-20">
        <header className="flex flex-col gap-5 border-b border-editorial-border pb-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-3"><AdminPeriodSelect value={period} onChange={setPeriod} /><Link href="/dashboard" className="text-xs font-bold uppercase tracking-wider text-editorial-muted transition-colors hover:text-editorial-red">Back to workspace <span aria-hidden="true">↗</span></Link></div>
        </header>
        {overview && subscription ? <AdminStatGrid stats={[{ label: "Agencies", value: String(overview.totalAgencies) }, { label: "In trial", value: String(overview.trialing) }, { label: "MRR", value: `${subscription.mrrZmw.toLocaleString()} ZMW` }, { label: "ARR", value: `${subscription.arrZmw.toLocaleString()} ZMW`, note: `${subscription.activeSubscribers} active subscribers · ${overview.churnRate.toFixed(1)}% churn` }]} /> : <AdminStatGrid loading stats={[]} />}
        <section className="grid gap-4 md:grid-cols-3" aria-label="Control plane status"><StatusCard icon={Activity} label="Platform readiness" value="Connected" note="Metrics are loaded from the control-plane APIs." tone="safe" /><StatusCard icon={Building2} label="Agencies under management" value={overview ? String(overview.totalAgencies) : "Loading…"} note="Current tenant count." tone="neutral" /><StatusCard icon={ShieldCheck} label="Administrative audit" value="Foundation active" note="Bootstrap access is server-gated." tone="safe" /></section>
