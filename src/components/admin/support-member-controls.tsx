...
export function SupportMemberControls({ sessionId }: { sessionId: string }) {
  ...
  const load = useCallback(async () => { const response = await fetch(`/api/admin/support-access/${sessionId}/members`); const data = await response.json(); setMembers(data.members || []); setMessage(response.ok ? "" : data.error || "Unable to load members."); }, [sessionId]);
