...
export async function reserveOrganizationOffer(input: { organizationId: string; offerId: string; paymentId: string }) {
  return db.$transaction(async (tx) => {
    const now = new Date();
    const grant = await tx.organizationOffer.findFirst({ where: { organizationId: input.organizationId, offerId: input.offerId, status: "ACTIVE", OR: [{ expiresAt: null }, { expiresAt: { gt: now } }] }, include: { offer: true }, orderBy: { createdAt: "asc" } });
