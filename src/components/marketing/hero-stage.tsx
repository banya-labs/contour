...
export function HeroStage() {
  ...
  const stats = [
    { value: "180+", label: "Active Mandates" },
    { value: "K 2.4B", label: "Portfolio Value" },
    { value: "5%", label: "Fixed Commission" },
        ...
        <div className="max-w-[1400px] mx-auto border-x border-editorial-black">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-editorial-black">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="px-6 sm:px-8 py-6 sm:py-8 flex flex-col justify-center items-start group hover:bg-editorial-hover transition-colors"
              >
                <span className="font-heading font-bold text-3xl sm:text-4xl lg:text-5xl tracking-tight text-editorial-black">
                  {stat.value}
                </span>
                <span className="mt-1 font-geist text-xs uppercase tracking-wider text-editorial-muted">
                  {stat.label}
                </span>
              </div>
            ))}
